...
section.appendChild(image);
  section.insertAdjacentHTML('beforeend', '<p>' + item["title"] + '</p>' + '<h1>' + item["price"] + ' руб.' + '</h1>' + '<button class="orderButton"> Заказать </button>' + '<button class="addToCartButton"> В корзину </button>' );
...
let popupText = document.getElementById('topDivOne')
 ...
 price.innerHTML = API.products[sectionId]["price"] + '<strong> руб. </strong>'
...
popupText.innerHTML = API.products[sectionId]["title"] + ' ' + API.products[sectionId]["price"] + '<strong> руб. </strong>'
popupImage.src = API.products[sectionId]["img"]
}
...
closeButtonOne.addEventListener('click', hidePopUp); })
